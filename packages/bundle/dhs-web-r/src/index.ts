...
export interface Config {
  ...
  trustedHosts: string[]
  /** Shared remote-service mode; makes the trusted host a browser-facing service. */
  remoteService?: boolean
  /** Optional external browser origin used for displayed/runtime URLs. */
  publicUrl?: string
}
...
export interface WebRuntimeValues {
  ...
  trustedHosts: string[]
  /** Whether this is a shared remote browser service. */
  remoteService?: boolean
  /** Optional external browser origin. */
  publicUrl?: string
}
 ...
 */
export function resolveLanTrust(bindHost: string, extra: readonly string[], remoteService = false, publicUrl?: string): WebRuntimeValues {
...
/** Resolve the browser-facing URL from the configured public origin or active Web server. */
function localWebUrl(ctx: Context): string {
  const runtime = ctx.get(WEB_RUNTIME_SERVICE) as WebRuntimeValues | undefined
  if (runtime?.publicUrl !== undefined && runtime.publicUrl !== '') return runtime.publicUrl
...
export function apply(ctx: Context, config: Config): void {
  const runtime = resolveLanTrust(ctx.webServer.host, config.trustedHosts, config.remoteService, config.publicUrl)
  // The loopback URL belongs to this host. Under SSH, the operator reaches it
  // through a local forwarding address that this process cannot derive.
  const handoffBrowser = config.openBrowser && !launchedThroughSsh(ctx)
  // Release dependent rows only after bind-dependent trust has been sampled once.
  ctx.provide(WEB_RUNTIME_SERVICE, runtime)
  if (runtime.remoteService) {
    ctx.on('webserver/index-inject', (table) => {
      table.push({ kind: 'global', name: '__DSH_REMOTE_SERVICE__', value: true })
    })
  }
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical browser URL of the DeepSeek Harness Web GUI serving this session.' },
        },
